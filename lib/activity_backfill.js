...
function applyRedeemMarks(pos, g) {
  ...
  // Polymarket /activity REDEEM often has outcome="" but outcomeIndex 0=Up, 1=Down
  if (!redeemUp && !redeemDown) {
    ...
    if (idxs.length) {
      ...
      if (has0 && !has1) redeemUp = true;
        ...
        // Ambiguous multi-index — prefer last redeem
        const last = idxs[idxs.length - 1];
        if (last === 0) redeemUp = true;
        else redeemDown = true;
      }
    }
  }
...
/** Normalize activity row outcome from outcomeIndex when string outcome missing. */
...
async function syncActivityLedger(state, opts = {}) {
  ...
  for (const [id, g] of byCond.entries()) {
    ...
    if (!stillOpen) {
      // Already settled: still allow filling missing Up/Down marks from redeem
  ...
  // Dump-only orphans often have no REDEEM — fill marks from Gamma once settled.
  try {
