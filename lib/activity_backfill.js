...
function applyRedeemMarks(pos, g) {
  ...
  if (pos.upMark != null && pos.downMark != null) return false;

  let redeemUp = (g.redeems || []).some((r) => String(r.outcome) === 'Up');
  let redeemDown = (g.redeems || []).some((r) => String(r.outcome) === 'Down');
  if (!redeemUp && !redeemDown) {
    const idxs = (g.redeems || [])
      .map((r) => Number(r.outcomeIndex))
      .filter((n) => n === 0 || n === 1);
    if (idxs.length) {
      const has0 = idxs.includes(0);
      const has1 = idxs.includes(1);
      if (has0 && !has1) redeemUp = true;
      else if (has1 && !has0) redeemDown = true;
      else {
...
/** Normalize activity outcome when the API only supplies binary outcomeIndex. */
function normalizeActivityOutcome(row) {
  if (!row || typeof row !== 'object') return row;
  const existing = String(row.outcome || '');
  if (existing === 'Up' || existing === 'Down') return row;
  const idx = Number(row.outcomeIndex);
  if (idx === 0) row.outcome = 'Up';
  else if (idx === 1) row.outcome = 'Down';
  return row;
}
...
function aggregateByCondition(activity) {
  ...
  for (const raw of activity || []) {
    const row = normalizeActivityOutcome(raw);
...
function tradeRowFromActivity(row, type) {
  ...
  return {
    ...
    activityId: row.id || row.activityId || null,
...
function activityTradeKey(row, type = null) {
  if (!row) return null;
  const activityId = row.activityId || row.id;
  if (activityId) return `id:${String(activityId)}`;
  const tx = String(row.txHash || row.transactionHash || row.orderId || '').toLowerCase();
  const cid = String(row.conditionId || '');
  const side = String(row.outcome || row.side || '');
  const typ = String(type || row.type || 'TRADE').toUpperCase();
  const size = Number(row.shares != null ? row.shares : row.size) || 0;
  const price = Number(row.price) || 0;
  const rawTs = row.timestamp != null ? row.timestamp : row.ts;
  const parsedTs = typeof rawTs === 'string' ? new Date(rawTs).getTime() : Number(rawTs);
  const tsSec = Number.isFinite(parsedTs)
    ? Math.floor((parsedTs >= 1e12 ? parsedTs : parsedTs * 1000) / 1000)
    : 0;
  if (!tx && !cid) return null;
  return [tx, cid, side, typ, size.toFixed(6), price.toFixed(6), tsSec].join('|');
}

function knownTradeTxHashes() {
  const set = new Set();
  for (const t of readJsonl('trades.jsonl') || []) {
    const key = activityTradeKey(t, t?.type);
    if (key) set.add(key);
  }
  return set;
}
...
async function syncActivityLedger(state, opts = {}) {
  ...
  for (const g of byCond.values()) {
    for (const b of g.buys) {
      const key = activityTradeKey(b, 'LIVE_BUY');
    ...
    for (const s of g.sells) {
      const key = activityTradeKey(s, 'LIVE_SELL');
      if (!key || known.has(key)) continue;
  ...
  for (const [id, g] of byCond.entries()) {
    ...
    if (!stillOpen) {
      // Preserve settled PnL, but allow a complete REDEEM row to fill missing marks.
      if (wasSettled && !mayRewriteSettled) {
        if ((pos.upMark == null || pos.downMark == null) && g.hasRedeem && !activityIncomplete) {
          if (applyRedeemMarks(pos, g)) {
            updated += 1;
            appendJsonl('settlement_log.jsonl', {
              ts: new Date().toISOString(),
              conditionId: id,
              slug: g.slug,
              title: g.title,
              realized: Number(pos.realizedPnl) || g.realized_pnl_usdc,
              fees: 0,
              reason: 'activity-sync-marks-catchup',
              source: 'polymarket-activity',
              buyUsdc: rnd(g.buyUsdc, 4),
              sellUsdc: rnd(g.sellUsdc, 4),
              redeemUsdc: rnd(g.redeemUsdc, 4),
              upMark: Number(pos.upMark),
              downMark: Number(pos.downMark),
            });
          }
        }
        continue;
  ...
  // Dump-only settlements may have no REDEEM row; recover missing binary marks from Gamma.
  try {
    ...
    for (const pos of Object.values(state.positions || {})) {
      if (!pos?.settled || !pos.conditionId) continue;
      if (pos.upMark != null && pos.downMark != null) continue;
      const end = Number(pos.windowEnd) || 0;
      if (end && nowSec < end + 60) continue;
      const marks = await resolveMarksForPosition(pos, nowSec, params, { force: true });
      if (!marks || marks.upMark == null || marks.downMark == null) continue;
      pos.upMark = marks.upMark;
      pos.downMark = marks.downMark;
      updated += 1;
      appendJsonl('settlement_log.jsonl', {
        ts: new Date().toISOString(),
        ...
        fees: 0,
        reason: 'gamma-marks-catchup',
        source: marks.source || 'gamma',
        upMark: pos.upMark,
        downMark: pos.downMark,
      });
    }
  } catch (_) {
    /* mark recovery is non-fatal */
  }
