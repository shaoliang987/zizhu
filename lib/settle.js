...
async function cancelMarketOrdersAll(state, conditionId, reason) {
  ...
  if (isLive()) {
    try {
      ...
      await cancelLiveOrdersForMarket(state, conditionId, reason);
    } catch (err) {
...
async function settleEndedPositions(state, nowSec = Math.floor(Date.now() / 1000)) {
  ...
  for (const pos of Object.values(state.positions || {})) {
    const end = Number(pos.windowEnd) || 0;
    if (end && nowSec >= end) {
      await cancelMarketOrdersAll(state, pos.conditionId, 'window ended');
    }
  }
  ...
  for (const pos of Object.values(state.positions || {})) {
    ...
    await cancelMarketOrdersAll(state, pos.conditionId, 'settling');
...
async function checkAndSettleOpenPositions(state, nowSec = Math.floor(Date.now() / 1000)) {
  ...
  for (const pos of Object.values(state.positions || {})) {
    ...
    const end = Number(pos.windowEnd) || 0;
    if (end && nowSec >= end) {
      await cancelMarketOrdersAll(state, pos.conditionId, 'settle-check');
    }
  }
