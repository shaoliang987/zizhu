...
async function tryTakerHedgeMissingLeg(state, market, heldSide) {
  ...
  const costCheck = completePairCostOk(pos, missingSide, ask.price, params);
  if (!costCheck.ok) {
    addLog(
      state,
      `[实盘补腿] 跳过 ${missingSide} @ $${ask.price} · ${costCheck.reason}` +
        (costCheck.pairCost != null ? ` pairCost=${Number(costCheck.pairCost).toFixed(4)}` : ''),
      'warning'
    );
    return { ok: false, ...costCheck, missingSide };
  }
    ...
    if (matched > 0 || hedged) {
      addLog(
        ...
        `[实盘补腿] TAKER BUY ${missingSide} ${matched || size} @ $${ask.price}` +
          ` · pairCost≈${Number(costCheck.pairCost).toFixed(4)} · ${market.slug}`,
      ...
      try {
        ...
        if (remaining > 1e-8) {
          if (orderId) {
            ...
            addLog(
              state,
              `[实盘平仓] GTC SELL ${side} ${remaining} @ $${bid.price} 已挂出（无 order id，下轮对账）`,
              'warning'
            );
          }
        }
...
async function auditMarketPairExposure(state, market, nowSec = Math.floor(Date.now() / 1000), depth = 0) {
  ...
  const defer = shouldDeferOrphanUnwind({
    ...
    orphanDetectedAtMs: pos?.orphanDetectedAt ? Date.parse(pos.orphanDetectedAt) : null,
  });
  ...
  if (invUp && !invDown) {
    ...
    if (hasDownRest && defer.force) {
      await cancelLiveOrdersForMarket(state, market.conditionId, 'pair audit: force cancel Down rest');
    }
    ...
    const forceUnwind =
      hedge.reason === 'pair cost too high' ||
    ...
    await cancelLiveOrdersForMarket(state, market.conditionId, 'pair audit: missing Down');
    if (pos) setPairInflight(pos, 'unwind pending');
    let unwind = { fullyClosed: upSh <= 1e-8, remaining: upSh };
    if (upSh > 1e-8) {
      unwind = await tryUnwindLegAll(state, market, 'Up', upSh, { dump: forceUnwind });
    }
  ...
  if (invDown && !invUp) {
    ...
    if (hasUpRest && defer.force) {
      await cancelLiveOrdersForMarket(state, market.conditionId, 'pair audit: force cancel Up rest');
    }
    ...
    await cancelLiveOrdersForMarket(state, market.conditionId, 'pair audit: missing Up');
    if (pos) setPairInflight(pos, 'unwind pending');
    let unwind = { fullyClosed: downSh <= 1e-8, remaining: downSh };
    if (downSh > 1e-8) {
      unwind = await tryUnwindLegAll(state, market, 'Down', downSh, { dump: forceUnwind });
    }
    if (!unwind.fullyClosed && (Number(unwind.remaining) || 0) > 1e-8) {
      if (pos) {
  ...
  if (!costCheck.ok) {
    ...
    await cancelLiveOrdersForMarket(state, market.conditionId, 'pair audit: cost too high');
  ...
  if (!skewPlan.skip && skewPlan.skew >= minSkew) {
    for (const o of skewPlan.cancel) {
      await cancelLiveOrder(state, o.id, 'pair audit: size skew');
    }
...
async function postOneLiveLeg(state, market, prep, paired) {
  ...
  try {
    ...
    if (!tracked.ok) {
      ...
      state.stats.skipped = (Number(state.stats.skipped) || 0) + 1;
      return { ok: false, side: prep.side, reason: tracked.reason };
    }
...
async function syncLiveQuotes(state, market, prepared, { paired = false } = {}) {
  ...
  const postResults = await Promise.all(
    prepared.map((prep) => postOneLiveLeg(state, market, prep, paired))
  );
