...
/** Apply one authenticated user-channel event idempotently. REST remains fallback. */
async function applyLiveUserEvent(state, event) {
  if (!event?.orderId) return { applied: false, reason: 'missing order id' };
  const order = (state.open_orders || []).find(
    (o) => o.live && String(o.id) === String(event.orderId)
  );
  if (!order) return { applied: false, reason: 'unknown order' };

  if (event.kind === 'order_cancelled') {
    if (order.status === 'open') {
      // Cancellation can race a final match. Keep funds/order tracked until the
      // authoritative cumulative size is fetched by the REST reconciliation.
      order.reconcilePending = true;
      order.reconcileError = 'user websocket cancellation awaiting final size';
      order.cancelEventAt = new Date().toISOString();
      order.updatedAt = new Date().toISOString();
    }
    return { applied: order.status === 'open', kind: event.kind, order, pendingReconcile: true };
  }

  let targetMatched;
  if (event.kind === 'order_fill_total') {
    targetMatched = Number(event.matchedTotal) || 0;
  } else if (event.kind === 'trade_fill') {
    if (event.tradeId) {
      if (!Array.isArray(state.liveWsSeenTrades)) state.liveWsSeenTrades = [];
      if (state.liveWsSeenTrades.includes(event.tradeId)) {
        return { applied: false, reason: 'duplicate trade' };
      }
      state.liveWsSeenTrades.push(event.tradeId);
      if (state.liveWsSeenTrades.length > 1000) state.liveWsSeenTrades.splice(0, 200);
    }
    // FOK/FAK response booking usually wins the race. A closed local order means
    // this trade event confirms the same fill and must not be added again.
    if (order.status !== 'open' || (Number(order.remaining) || 0) <= 1e-8) {
      return { applied: false, reason: 'already booked' };
    }
    targetMatched = rnd((Number(order.sizeMatchedBooked) || 0) + (Number(event.size) || 0), 6);
  } else {
    return { applied: false, reason: 'unsupported event' };
  }

  const previous = Number(order.sizeMatchedBooked) || 0;
  const capped = Math.min(Number(order.originalSize) || targetMatched, targetMatched);
  if (!(capped > previous + 1e-9)) return { applied: false, reason: 'already booked' };
  const price = Number(event.price) || Number(order.limit);
  const isSell = String(order.side || 'BUY').toUpperCase() === 'SELL';
  let booked;
  if (isSell) {
    const delta = rnd(capped - previous, 6);
    booked = bookLiveSellFill(state, marketStubFromOrder(order), order.outcome, delta, price);
    if (booked.ok) {
      order.sizeMatchedBooked = capped;
      order.filled = capped;
      order.remaining = rnd(Math.max(0, (Number(order.originalSize) || 0) - capped), 6);
      if (order.remaining <= 1e-8) order.status = 'filled';
    }
  } else {
    booked = bookLiveFill(state, order, capped, price, order.quoteMode, null);
    if (booked.ok && order.paired) await maybePostPendingPairLegAfterFill(state, order);
  }
  order.updatedAt = new Date().toISOString();

  const pos = state.positions[order.conditionId];
  if (pos) {
    const { refreshInventoryState } = require('./inventory_state');
    const marketOrders = liveOrdersForMarket(state, order.conditionId);
    const transition = refreshInventoryState(pos, marketOrders);
    if (transition?.changed) {
      addLog(
        state,
        `[实盘库存] ${order.slug || order.conditionId} · ${transition.previous || 'init'} → ${transition.current}`,
        'info'
      );
    }
  }
  return { applied: Boolean(booked?.ok), kind: event.kind, order, booked };
}

/**
 * Enter a new pair with two sequential FOK taker orders.
 * Both books/depth/fees are checked before the high-priced leg is sent. The
 * second leg is sized from the first leg's confirmed fill, never the plan size.
 */
async function executeSafeTakerPair(state, market, prepared = []) {
  const params = loadParams();
  const upPrep = prepared.find((p) => p.side === 'Up');
  const downPrep = prepared.find((p) => p.side === 'Down');
  if (!upPrep || !downPrep) return { ok: false, reason: 'pair incomplete' };

  const { getBestAsk } = require('./book');
  let upAsk;
  let downAsk;
  try {
    [upAsk, downAsk] = await Promise.all([
      getBestAsk(market.upTokenId),
      getBestAsk(market.downTokenId),
    ]);
  } catch (err) {
    return { ok: false, reason: `pair ask refresh failed: ${err.message}` };
  }
  if (!(upAsk?.price > 0) || !(downAsk?.price > 0)) {
    return { ok: false, reason: 'pair ask unavailable' };
  }

  const feeRate = Number(params.taker_fee_rate);
  const rate = Number.isFinite(feeRate) && feeRate >= 0 ? feeRate : undefined;
  const upPerShare = buyCostWithFee(1, Number(upAsk.price), rate).cost;
  const downPerShare = buyCostWithFee(1, Number(downAsk.price), rate).cost;
  const pairCost = rnd(upPerShare + downPerShare, 6);
  const normalMax = Number(params.pair_sum_max) || 0.985;
  if (pairCost > normalMax + 1e-9) {
    return { ok: false, reason: 'initial taker pair cost too high', pairCost, normalMax };
  }
  ...
  const alreadySpent = Math.max(
    (Number(pos?.upCost) || 0) + (Number(pos?.downCost) || 0),
    Number(pos?.windowBuyUsdc) || 0
  );
  const room = Math.max(0, maxMarket - alreadySpent);
  const commonSize = rnd(Math.min(
    Number(upPrep.shares) || 0,
    Number(downPrep.shares) || 0,
    Number(upAsk.size) || 0,
    Number(downAsk.size) || 0,
    chunk,
    maxTrade / Math.max(Number(upAsk.price), Number(downAsk.price)),
    room / Math.max(pairCost, 1e-9)
  ), 4);
  if (commonSize + 1e-9 < minSh) {
    return { ok: false, reason: 'insufficient common taker depth', commonSize, minSh };
  }

  // Recheck authoritative collateral against taker prices and both fees. The
  // generic executor's earlier estimate may have been based on maker quotes.
  const actualNeed = rnd(pairCost * commonSize, 4);
  let accountCash;
  try {
    const { fetchAccountCashUsdc } = require('./account');
    accountCash = await fetchAccountCashUsdc({ force: true });
  } catch (err) {
    return { ok: false, reason: `safe taker cash check failed: ${err.message}` };
  }
  if (!(Number(accountCash) >= actualNeed - 1e-9)) {
    return { ok: false, reason: 'insufficient cash for both taker legs', actualNeed, accountCash };
  }

  const first = Number(upAsk.price) >= Number(downAsk.price)
    ? { side: 'Up', tokenId: market.upTokenId, ask: upAsk }
    : { side: 'Down', tokenId: market.downTokenId, ask: downAsk };
  const { placeLiveTakerBuy } = require('./executor');
  let resp;
  try {
    resp = await placeLiveTakerBuy(first.tokenId, commonSize, first.ask.price);
  } catch (err) {
    return { ok: false, reason: `first taker leg failed: ${err.message}`, firstSide: first.side };
  }

  const matched = parseBuyMatchedFromResponse(resp);
  const remoteId = extractOrderId(resp);
  if (!(matched > 0)) {
    tripLiveCircuitBreaker(state, market, {
      reason: 'first FOK BUY returned no confirmed fill',
      remaining: commonSize,
      side: first.side,
    });
    if (remoteId) {
      trackUncertainLiveBuy(state, {
        market,
        prep: { side: first.side, tokenId: first.tokenId, shares: commonSize, limit: first.ask.price, quoteMode: 'taker' },
        orderId: remoteId,
        paired: true,
        reason: 'FOK fill not confirmed',
      });
    }
    saveState(state);
    return { ok: false, reason: 'first taker fill uncertain', breaker: true, firstSide: first.side };
  }

  const localId = remoteId || `filled-fok-${market.conditionId}-${Date.now()}`;
  const tracked = trackLiveOrder(state, {
    market,
    side: first.side,
    tokenId: first.tokenId,
    shares: commonSize,
    limitPrice: first.ask.price,
    orderId: localId,
    quoteMode: 'taker',
    paired: true,
    plannedPrice: first.ask.price,
    marketAsk: first.ask.price,
    sizeMatched: matched,
    makingAmount: parseUsdc(resp.makingAmount),
  });
  if (!tracked.ok) {
    tripLiveCircuitBreaker(state, market, {
      reason: 'confirmed first FOK BUY could not be booked',
      remaining: matched,
      side: first.side,
    });
    saveState(state);
    return { ok: false, reason: tracked.reason, breaker: true, firstSide: first.side };
  }

  addLog(
    state,
    `[实盘双TAKER] 第一腿 ${first.side} ${matched} @ $${first.ask.price} 已确认 · 开始等量补腿`,
    'warning'
  );
  const hedge = await tryTakerHedgeMissingLeg(state, market, first.side);
  if (!hedge.hedged) {
    const recovery = await auditMarketPairExposure(state, market);
    return { ok: false, reason: hedge.reason || 'second taker leg failed', firstMatched: matched, hedge, recovery };
  }
  return { ok: true, mode: 'safe-taker-pair', firstSide: first.side, firstMatched: matched, hedge, pairCost };
}
...
async function tryTakerHedgeMissingLeg(state, market, heldSide) {
  ...
  const { getBestAsk, getBestBid } = require('./book');
  const tokenId = missingSide === 'Up' ? market.upTokenId : market.downTokenId;
  const heldTokenId = heldSide === 'Up' ? market.upTokenId : market.downTokenId;
  const ask = await getBestAsk(tokenId);
  if (!ask || !(ask.price > 0)) {
    return { ok: false, reason: 'no ask', missingSide };
  }

  let costCheck = canTakerHedgePairCost(heldSide, pos, ask.price, params);
  if (!costCheck.ok && costCheck.completeSum <= costCheck.emergencyMax + 1e-9) {
    const heldBid = await getBestBid(heldTokenId).catch(() => null);
    costCheck = canTakerHedgePairCost(heldSide, pos, ask.price, params, heldBid?.price);
  }
  if (!costCheck.ok) {
    addLog(
      state,
      `[实盘补腿] 跳过 ${missingSide} @ $${ask.price} · ${costCheck.reason}` +
        (costCheck.completeSum != null
          ? ` pairCost≈${Number(costCheck.completeSum).toFixed(4)} > normal ${costCheck.maxSum}` +
            (costCheck.emergencyMax ? ` / emergency ${costCheck.emergencyMax}` : '')
          : ''),
      'warning'
    );
    ...
    if (matched > 0 || hedged) {
      addLog(
        ...
        `[实盘补腿] TAKER BUY ${missingSide} ${matched || size} @ $${ask.price}` +
          ` · pairCost≈${Number(costCheck.completeSum).toFixed(4)}` +
          (costCheck.mode === 'emergency'
            ? ` · 紧急补腿 penalty/sh=$${costCheck.emergencyPenaltyPerShare}` +
              ` < unwindLoss/sh=$${costCheck.unwindLossPerShare}`
            : '') +
          ` · ${market.slug}`,
      ...
      try {
        ...
        if (remaining > 1e-8) {
          if (orderId) {
            ...
            addLog(
              state,
              `[实盘平仓] GTC 无 order id · 状态不明，停止重复卖出 ${side} ${remaining} @ $${bid.price}`,
              'error'
            );
            tripLiveCircuitBreaker(state, market, {
              reason: 'submitted SELL returned no order id',
              remaining,
              side,
            });
            saveState(state);
...
async function auditMarketPairExposure(state, market, nowSec = Math.floor(Date.now() / 1000), depth = 0) {
  ...
  if (invUp && !invDown) {
    ...
    if (upSh > 1e-8) {
      const dumpUnwind = Boolean(defer.force);
...
/** Shares needed to neutralize the current inventory imbalance. */
function pendingPairHedgeShares(upShares, downShares) {
  return rnd(Math.abs((Number(upShares) || 0) - (Number(downShares) || 0)), 6);
}
...
async function maybePostPendingPairLegAfterFill(state, order) {
  ...
  // Hedge only the live inventory imbalance. Using the original planned size here
  // overbuys the deferred leg when the first leg has filled only partially.
  const imbalance = pendingPairHedgeShares(upSh, downSh);
  if (!(imbalance > 1e-8)) {
    delete pos.pendingPairLegs;
    return { posted: false, reason: 'balanced or flat' };
  }
  const hedgePrep = { ...prep, shares: imbalance };

  // A stored maker quote may have become marketable while the first leg filled.
  // Do not let a nominal GTC maker hedge cross the current ask and incur an
  // unbudgeted taker fee; the pair audit can instead use the strict taker gate.
  const { getBestAsk } = require('./book');
  let ask;
  try {
    ask = await getBestAsk(prep.tokenId);
  } catch (err) {
    addLog(state, `[实盘配对] 无法刷新 ${missingSide} ask: ${err.message}`, 'warning');
    return { posted: false, reason: 'fresh ask failed', missingSide };
  }
  if (!ask || !(Number(ask.price) > 0)) {
    return { posted: false, reason: 'no fresh ask', missingSide };
  }
  if (Number(prep.limit) >= Number(ask.price) - 1e-12) {
    addLog(
      state,
      `[实盘配对] 跳过过期 maker 补腿 ${missingSide} @ $${prep.limit} · 当前 ask $${ask.price}`,
      'warning'
    );
    return { posted: false, reason: 'maker limit crosses fresh ask', missingSide };
  }
  ...
  addLog(
    state,
    `[实盘配对] ${order.outcome} 成交 · 立即挂 ${missingSide} ${imbalance} @ $${prep.limit} · ${market.slug}`,
    'info'
  );
  const result = await postOneLiveLeg(state, market, hedgePrep, true);
  if (result.ok) {
