...
async function auditMarketPairExposure(state, market, nowSec = Math.floor(Date.now() / 1000), depth = 0) {
  ...
  if (invUp && !invDown) {
    ...
    if (hasDownRest && defer.defer && !lossGate.exceeded) {
      return { hedged: false, ok: true, reason: defer.reason || 'Down hedge resting' };
    }
    if (hasDownRest && (defer.force || lossGate.exceeded)) {
    ...
    if (upSh > 1e-8) {
      const dumpUnwind = Boolean(defer.force && defer.reason !== 'hedge rest timeout');
  ...
  if (invDown && !invUp) {
    ...
    if (hasUpRest && defer.defer && !lossGate.exceeded) {
      return { hedged: false, ok: true, reason: defer.reason || 'Up hedge resting' };
    }
    if (hasUpRest && (defer.force || lossGate.exceeded)) {
      const ok = await cancelRiskOrdersOrBreak(
        state,
        market,
