...
const { rnd, buyCostWithFee } = require('./fees');
const {
  pairEntryBlocked,
  pairEntryRiskGate,
  rebalanceRiskGate,
...
function evaluateMarket(
  ...
  // Taker: fee-inclusive all-in must also clear pair_sum_max (matches executeSafeTakerPair).
  if (mode === 'taker') {
    const feeRate = Number(params.taker_fee_rate);
    const rate = Number.isFinite(feeRate) && feeRate >= 0 ? feeRate : undefined;
    const pairCost = rnd(
      buyCostWithFee(1, pu, rate).cost + buyCostWithFee(1, pd, rate).cost,
      6
    );
    if (pairCost > maxSum + 1e-9) {
      return {
        action: 'SKIP',
        reason: `taker pairCost ${pairCost.toFixed(4)} > max ${maxSum}`,
        pairSum,
        pairCost,
        askSum: pairQuotes?.askSum ?? null,
        bidSum: pairQuotes?.bidSum ?? null,
