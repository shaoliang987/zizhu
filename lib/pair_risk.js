...
function shouldDeferOrphanUnwind({
  ...
  const entryEnd = Number(params.entry_end_sec) || 240;
  const graceSec = orphanGraceSec(params);
  const detectedAtMs = Number(orphanDetectedAtMs);
  const orphanAgeSec = Number.isFinite(detectedAtMs) && detectedAtMs > 0
    ? Math.max(0, now - detectedAtMs / 1000)
    : null;
  const graceEnd = entryEnd + graceSec;
  ...
  if (up > 1e-8 && down <= 1e-8) {
    ...
    const graceLeft = orphanAgeSec == null ? graceEnd - t : graceSec - orphanAgeSec;
    if (graceLeft > 0) {
      return { defer: true, reason: `orphan Up grace ${Math.max(0, Math.ceil(graceLeft))}s` };
    }
  }
  if (down > 1e-8 && up <= 1e-8) {
    ...
    const graceLeft = orphanAgeSec == null ? graceEnd - t : graceSec - orphanAgeSec;
    if (graceLeft > 0) {
      return { defer: true, reason: `orphan Down grace ${Math.max(0, Math.ceil(graceLeft))}s` };
    }
  }
  return { defer: false };
}
...
function completePairCostOk(pos, missingSide, askPrice, params = loadParams()) {
  ...
  const pairCost = rnd(heldAvg + ask, 4);
  if (pairCost > maxSum + feeBuf + 1e-9) {
    return { ok: false, reason: 'pair cost too high', pairCost, maxSum, heldAvg, ask };
  }
  return { ok: true, pairCost, maxSum, heldAvg, ask };
}
...
function canTakerHedgePairCost(heldSide, pos, missingAskPrice, params = loadParams()) {
  const completeSum = completePairCost(heldSide, pos, missingAskPrice);
  if (completeSum == null) return { ok: false, reason: 'no held leg' };
  const maxSum = Number(params.pair_sum_max) || 0.99;
  const feeBuf = Number(params.pair_hedge_fee_buffer);
  const buffer = Number.isFinite(feeBuf) && feeBuf >= 0 ? feeBuf : 0.015;
  if (completeSum <= maxSum + buffer + 1e-9) {
    return { ok: true, completeSum, maxSum, buffer };
  }
  return { ok: false, completeSum, maxSum, buffer, reason: 'pair cost too high' };
}
...
function pairCostFeeBuffer(params = loadParams()) {
  const feeBuf = Number(params.pair_hedge_fee_buffer);
  return Number.isFinite(feeBuf) && feeBuf >= 0 ? feeBuf : 0.015;
}
