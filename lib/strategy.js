...
const DEFAULTS = {
  ...
  /** After entry_end, wait before orphan-unwind if hedge order was cancelled. */
  orphan_grace_sec: 90,
  ...
  pair_taker_hedge: 1,
  /** Legacy buffer; never expands pair_sum_max. */
  pair_hedge_fee_buffer: 0,
  /** Emergency taker hedge cap, used only when cheaper than unwinding the orphan. */
  emergency_pair_sum_max: 0.992,
  /** Live maker entry: rest only the higher-priced leg, then hedge the cheaper leg after fill. */
  live_high_leg_first: 1,
  /** Trigger forced orphan handling when mark-to-bid loss reaches this USDC amount. */
  max_orphan_loss_usdc: 0.35,
  /** Min seconds one-sided before loss gate may dump (hedge-rest still blocks earlier). */
  orphan_loss_min_sec: 45,
  /** Skip loss-based dump while more than this many seconds remain before window end. */
